...
import { SprintContext, SprintTask } from './sprintReader';

export class SprintStatusUpdater {
    ...
    async updateTaskStatus(
        ...
        taskName: string, 
        newStatus: SprintTask['issuestatus']
    ): Promise<boolean> {
        try {
    ...
    private async addStatusToTask(
        lines: string[], 
        taskName: string, 
        status: SprintTask['issuestatus']
    ): Promise<boolean> {
        let inTargetTask = false;
        let taskIndent = 0;
        let insertIndex = -1;
    ...
    async markTaskInProgress(context: SprintContext, task: SprintTask): Promise<boolean> {
        return await this.updateTaskStatus(context, task.name, 'inprogress');
    }

    async markTaskDone(context: SprintContext, task: SprintTask): Promise<boolean> {
        return await this.updateTaskStatus(context, task.name, 'done');
    }

    async markTaskBlocked(context: SprintContext, task: SprintTask, reason?: string): Promise<boolean> {
    ...
    async verifyOutputFileExists(task: SprintTask): Promise<boolean> {
        if (!task.outputfile) {
